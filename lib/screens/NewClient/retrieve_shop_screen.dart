...
import 'package:thecut/screens/NewClient/directions_repository.dart';
import 'package:thecut/screens/NewClient/shop_detail.dart';
...
class _RetrievePageState extends State<RetrievePage> with AutomaticKeepAliveClientMixin{
    ...
    return Stack(alignment: Alignment.bottomCenter, children: [
      GoogleMap(
        ...
        onMapCreated: (GoogleMapController mapController) async {
          ...
          setState(() {
            currentCam = CameraPosition(target: LatLng(getCurrentPosition().latitude, getCurrentPosition().longitude), zoom: 13);
            ;
          });
          ...
          child: Padding(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return MapShopCard(shopDetails:currentShop,curLocation: getCurrentPosition(),);
                },
              )),
        ),
      ),
  ...
  populateNearByShops() async {
    ...
    FirebaseFirestore.instance
      ...
      querySnapshot.docs.forEach((doc) {
        ...
        setState(() {
          listOfShops.add(Shop(doc['uid'], doc['location'] as GeoPoint,
              doc['name'], doc['phone']));
        });
    ...
    setState(() {
      _markers.add(Marker(
        ...
        onTap: () async {
          ...
          final origin = LatLng(getCurrentPosition().latitude, getCurrentPosition().longitude);
  ...
  CameraPosition initialCameraPosition() {
    ...
    return CameraPosition(
        target: LatLng(appProv.currentpos.latitude,appProv.currentpos.longitude), zoom: 16);
  }

  Position getCurrentPosition(){
    final appProv=Provider.of<ApplicationProvider>(context,listen:false);

    return appProv.currentpos;
  }
...
class MapShopCard extends StatefulWidget {
  final Shop shopDetails;
  final Position curLocation;
  const MapShopCard({Key? key, required this.shopDetails, required this.curLocation,}) : super(key: key);

  @override
  _MapShopCardState createState() => _MapShopCardState();
}

class _MapShopCardState extends State<MapShopCard> {

  Directions? _info;

  displayDistance(Position origin,LatLng dest) async {
    //final origin = LatLng(currentpos.latitude, currentpos.longitude);
    final directions = await DirectionsRepository()
        .getDirections(origin: LatLng(origin.latitude,origin.longitude), destination: dest);
    if(mounted) setState(() => _info = directions);
  }
  ...
  void initState() {
    ...
    super.initState();
    GeoPoint dest=widget.shopDetails.location;
    displayDistance(widget.curLocation,LatLng(dest.latitude,dest.longitude));
  }
        ...
        onTap: () {
          Navigator.push(context,
              MaterialPageRoute(builder: (ctx) {
                return AboutShop(shopId: widget.shopDetails.uid);
              }));
        },
        child: Card(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            if(_info!=null)Text(_info!.totalDistance)
                          ],
                        )
                      ],
                    ),
